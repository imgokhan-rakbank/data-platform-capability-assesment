...
| **9** | **Is there a single, authoritative Silver catalog / environment — or do multiple Silver catalogs / databases exist in parallel (e.g., silver_dev, silver_dev_v2, silver_prod)?** | **Confirmed critical gap: Two Silver catalogs run in parallel. `silver_dev` (confirmed from `pyhsical_silver_2.csv`) has 408 tables across 18 schemas with 0 SCD2 tables and 249 data type defects. `silver_dev_v2` (from `pyhsical_silver.csv`) has 120 tables across 16 schemas with 11 SCD2 tables. Only 5 schemas overlap (account, credit_card, customer, deposits, loan). GL, Trade, Channels, Collateral, Application, and Risk Scoring domains exist only in `silver_dev` with no SCD2. Neither catalog is complete alone. No decommission or migration plan documented.** |
| **10** | **Are the two Silver sub-layers (Silver ODS and Silver Analytical) physically separated into distinct catalogs/databases as defined in the architecture, or co-located in a single catalog?** | **Architecture patterns define two distinct namespaces: `silver_ods.{source_system}.*` and `silver_analytical.{domain}.*`. In `silver_dev_v2`, both co-exist in a single catalog: source-specific ODS schemas (finacle, flexcube, fis, prime4, erp, eximbills — 103 tables) and domain-named analytical schemas (customer, account, loan, credit_card — 45 tables). `silver_dev` uses only domain-analytical schemas with no ODS pattern. The prescribed namespace separation is not implemented in either catalog.** |
...
| Known Gaps / Limitations | Documentation is basic, not communicated well, nothing is enforced. **Critical: Two Silver catalogs confirmed in parallel — `silver_dev` (408 tables, 0 SCD2, broader domain coverage) and `silver_dev_v2` (120 tables, 11 SCD2, ODS pattern). Only 5 schemas overlap. GL, Trade, Channels, Collateral are only in `silver_dev` with no SCD2. No migration or decommission plan. Silver ODS/Analytical sub-layer boundary is not physically enforced through separate catalog namespacing.** |
